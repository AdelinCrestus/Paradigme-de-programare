...
colturi([nw,ne,sw,se]).
movePriority(Player, Board, Move1, 0) :- positions(L), modifyBoard(Board, Move1, Player, L, [], NewBoard), getBoardResult(NewBoard, Player),!.
movePriority(Player, Board, Move2, 1 ) :- getNextPlayer(Player, LastPl), positions(L), modifyBoard(Board, Move2, LastPl, L, [], NewBoard ), getBoardResult(NewBoard, LastPl),!.
movePriority(_, Board, Move3, 2) :- empty_board(Board), colturi(C), member(Move3, C), !.
movePriority(Player, Board, Move4, 3) :- \+ getPos(Board, _ ,Player ),getNextPlayer(Player, OpPlayer), getPos(Board, c, OpPlayer), colturi(L), member(Move4, L),!.
movePriority(Player, Board, c, 3) :- \+ getPos(Board, _ ,Player ),getPos(Board, c, ''), !.
movePriority(Player, Board, Move, 4) :- positions(L), member(Move, L), member(Move2, L),  getPos(Board, Move, ''), modifyBoard(Board, Move, Player, L, [], NewBoard), getPos(NewBoard, Move2, ''), modifyBoard(NewBoard, Move2, Player, L, [], NBB), getBoardResult(NBB, Player), !.
movePriority(_, _, Move6, 5) :- colturi(C), member(Move6, C), !.
...
bestIndividualMoves(P, Board, Moves) :- positions(L),findall((Y,X), (member(X, L), getPos(Board, X, ''), movePriority(P,Board, X, Y) ), Bag), sortMoves(Bag,Moves).
...
narrowGreedy(S, Move) :- getNextPlayer(S, Player), getUBoard(S, UB),getNextAvailableBoards(S, NextBoardsPoss), length(NextBoardsPoss, 1) ,bestIndividualMoves(Player, UB, Moves), nth0(0, Moves, Upos), getBoard(S, Upos, Board), bestIndividualMoves(Player, Board, Moves2), nth0(0, Moves2, Move), !.
narrowGreedy(S, (Upos,Move)) :- getNextPlayer(S, Player), getUBoard(S, UB), bestIndividualMoves(Player, UB, Moves), nth0(0, Moves, Upos), getBoard(S, Upos, Board), bestIndividualMoves(Player, Board, Moves2), nth0(0, Moves2, Move).
...
bestMoves(S, Moves) :- getNextPlayer(S, Player),getNextAvailableBoards(S, NextBoardsPoss),length(NextBoardsPoss, 1), findall(X, (member(Upos, NextBoardsPoss), getBoard(S, Upos, Board), bestIndividualMoves(Player, Board, MovesI), member(X, MovesI)), Moves). 
bestMoves(S, Moves) :- getNextPlayer(S, Player),getNextAvailableBoards(S, NextBoardsPoss), findall((Upos,X), (member(Upos, NextBoardsPoss), getBoard(S, Upos, Board), bestIndividualMoves(Player, Board, MovesI), member(X, MovesI)), Moves).
bestMoves(_, _) :- false.
...
% bestMoves/2.
greedy(S, Move) :- bestMoves(S,Moves), nth0(0, Moves, Move).
greedy(_, _) :- false.
