...
mapSingleHelper f cpy acc x 
    | x >= length cpy = acc
    | x < length cpy = mapSingleHelper f cpy ((map (\t -> if  (eljust (findIndex (\o -> o == t) cpy)) == x then f t else t) cpy) : acc) (x + 1)
mapSingle :: Eq a => (a -> a) -> [a] -> [[a]]
mapSingle f xs = reverse (mapSingleHelper f xs [] 0)
...
--partitions (t:xs) = [t:y | y <- partitions xs] ++ (partitions xs) ++ [t]
--partition xs = [[ o | f <-  (partitions (tail xs)),  o <- (mapSingle (\x -> t : x) f) ] ++ [[[x]] | x <- xs] ++ [[[x | x <- xs ]]]]
--partition (t:xs) = [ o | f <-  (partitions xs),  o <- (mapSingle (\x -> t : x) f) ] ++ [[[x]] | x <- t:xs] ++ [[[x | x <- t:xs ]]]
...
isModule :: Ord a
         ...
         -> Graph a
         -> Bool
isModule set graph = length (S.map (\x -> S.filter (\y -> S.notMember y set == True ) (outNeighbors x graph)) (set)) == 1 && length (S.map (\x -> S.filter (\y -> S.notMember y set == True ) (inNeighbors x graph)) (set)) == 1  
...
isModularPartition :: Ord a
                   ...
                   -> Graph a
                   -> Bool
isModularPartition partition graph = length y == 1 && S.member True y
    where y = (S.map (\x -> isModule x graph) partition)
