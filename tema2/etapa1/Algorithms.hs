...
{- searchHelper node visited  
    | S.member node visited == True = visited
    | otherwise = S.insert node visited
-}
searchHelper :: Ord a
        => ([a] -> [a] -> [a])
        -> a
        ...
        -> [a]
        -> [a]
searchHelper f node graph visited list
    | null list == True = visited 
    | S.member node (S.fromList visited) == True = searchHelper f (head x) graph visited (tail list)
    | otherwise = searchHelper f (head x) graph (visited ++ [node]) x
    where x = tail (f list (S.toList (outNeighbors node graph)))
...
search f node graph = searchHelper f node graph [] [node]
...
dfs = search (\x y -> (([head x] ++ y) ++ (tail x)))
...
countIntermediate from to graph
    | S.member to  (S.fromList (dfs from graph)) == False = Nothing
    | otherwise = Just (length (fst (span (\x -> x /= to) (bfs from graph))) - 1, length (fst (span (\x -> x /= to) (dfs from graph))) - 1) 
