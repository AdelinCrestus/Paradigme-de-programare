...
removeNode node graph = fromComponents  (S.toList (S.filter (\x -> x /= node) (nodes graph) )) (S.toList (S.filter (\x -> snd x /= node && fst x /= node) (edges graph) ))
{-
    *** TODO ***
...
splitNode old news graph = removeNode old (fromComponents ((S.toList (nodes graph)) ++ news) ((S.toList (edges graph)) ++ (S.toList (S.cartesianProduct (inNeighbors old graph) (S.fromList news))) ++ (S.toList (S.cartesianProduct (S.fromList news) (outNeighbors old graph)))))
    {-li = lista care intra - punem news in loc de old(pe poz 2)
     - lo = lista care ies - pe poz 1
