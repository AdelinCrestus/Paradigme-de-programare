...
(define (key n)
  (let* ((quadruple (get-nth-quadruple n)) (e (second quadruple)) (f (third quadruple)))
    (map (λ (x) (modulo (apply x (list e f)) 27)) (list a1 b1 c1 a2 b2 c2 a3 b3 c3))
    )
  )
...
(define (conversie x)
  (if (equal? x #\ )
      0
      (add1 (- (char->integer x) (char->integer #\a )))
      ))
      
(define (message->codes message)
  (map conversie (string->list message) )
  )
...
(define (conversie2 x)
  (if (zero? x)
      #\ 
      (integer->char (sub1 (+ x (char->integer #\a ))))
      ))
(define (codes->message codes)
  (list->string (map conversie2 codes)))
...
(define (extend-key key size)
  (let iter ((key key) (acc '()) (parcurgere key) (size size))
    (cond
      ((= size 0) (reverse acc))
      ((null? parcurgere) (iter key acc key size))
      ((iter key (cons (car parcurgere) acc) (cdr parcurgere) (sub1 size)))
      )))
...
(define (abstractizare mod x y)
  (λ (message key) (y (map (λ (x y) (modulo (mod x y) 27)) (x message) (extend-key key (length (x message)))))))

(define (identitate x) x)

(define encrypt-codes (abstractizare + identitate identitate))
(define decrypt-codes (abstractizare - identitate identitate))
...
(define encrypt-message
  (abstractizare + message->codes codes->message))
