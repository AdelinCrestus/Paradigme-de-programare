...
(define ppt-stream-in-tree-order
  (let iterator ( (coada '((3 4 5)))) 
    (let ( (vecini (list  (multiply T1 (car coada)) (multiply T2 (car coada)) (multiply T3 (car coada)))) )
    (stream-cons (car coada) (iterator (cdr (append coada vecini)) ))
    )
  ))
...
(define (pairs G H)
 (let it ( (indexl 1) (indexc 1) (copie G) (G1 G) (H1 H))
   (cond
     ( (= indexl indexc) (stream-cons (cons (stream-first G1) (stream-first H1)) (it 1 (add1 indexc) copie copie (stream-rest H1) )))
     ( (< indexl indexc) (stream-cons (cons (stream-first G1) (stream-first H1)) (it (add1 indexl) indexc copie (stream-rest G1) H1)))
  )
   ))
...
(define (odd-numbers-from-x x)
  (let it (( nr x ))
    (stream-cons nr (it (+ 2 nr)))
    )
  )

(define odd-numbers-from-1 (odd-numbers-from-x 1))
(define odd-numbers-from-3 (odd-numbers-from-x 3))

(define gh-pairs-stream
 (stream-filter (λ(x)
                       (or (< (gcd (cdr x) (car x) ) 2))) (pairs odd-numbers-from-1 odd-numbers-from-3)
  ))
...
(define ppt-stream-in-pair-order
  (stream-map (λ(x) (list (* (car x) (cdr x)) (/ (* (+ (cdr x) (car x)) (- (cdr x) (car x))) 2) (/ (+ (sqr (cdr x)) (sqr (car x))) 2))) gh-pairs-stream)
)
